...
public class BookLauncher {
    ...
    public static void setGameProvider(GameProvider provider) {
        if (provider != null) {
            BookLauncher.provider = provider;
        }
    }


    public static QuiltClassLoader getQuiltClassLoader() {
        return BookLoader.classLoader;
    }
    ...
    public static InputStream getResourceAsStream(String name) {
        ...
        InputStream stream = BookLoader.classLoader.getResourceAsStream(name);
        ...
        for (ModVessel vessel : Loader.getModVessels()) {
            InputStream vesselResolved = vessel.getClassLoader().getResourceAsStream(name);
    ...
    /**
     * This method uses all our available classloaders to search for class bytes via {@link AbstractBookClassLoader}.
    ...
    public static byte[] getClassBytesMixin(String name, boolean transform) {
        AbstractBookClassLoader classLoader = getQuiltClassLoader();
        byte[] classBytes = classLoader.getClassBytesMixin(name, transform);

        if (classBytes == null) {
            for (ModVessel vessel : Loader.getModVessels()) {
                AbstractBookClassLoader loader = vessel.getClassLoader();
    ...
    public static boolean isClassLoaded(String name) {
        ...
        for (ModVessel vessel : Loader.getModVessels()) {
            ...
            if (vessel.getClassLoader().isClassLoaded(name)) {
                return true;
            }
        }
     ...
     * classloaders such as the {@link ModClassLoader}s.
     *
    ...
    public static URL[] getURLs() {
        ...
        for (ModVessel vessel : Loader.getModVessels()) {
            urls.addAll(Arrays.asList(vessel.getClassLoader().getURLs()));
        }
     ...
     */
    public static Class<?> loadVesselClass(String name) throws ClassNotFoundException {
        for (ModVessel vessel : Loader.getModVessels()) {
            try {
                AbstractBookClassLoader classLoader = vessel.getClassLoader();
                if (vessel.getClassLoader().isClassLocatable(name)) {
    ...
    public static Class<?> loadClass(String name, boolean tryMainClassLoader) throws ClassNotFoundException {
        if (tryMainClassLoader) {
            ...
            if (mainClassLoader.isClassLocatable(name)) {
    ...
    public static Class<?> findClass(String name, boolean initialize) throws ClassNotFoundException {
        try {
            ...
            for (ModVessel vessel : Loader.getModVessels()) {
                try {
                    return Class.forName(name, initialize, vessel.getClassLoader());
