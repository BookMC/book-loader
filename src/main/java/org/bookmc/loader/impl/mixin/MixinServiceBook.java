...
public class MixinServiceBook implements IMixinService, IClassProvider, IClassBytecodeProvider, ITransformerProvider, IClassTracker {
    ...
    public ClassNode getClassNode(String name, boolean runTransformers) {
        // We use 0 for our flags to avoid re-entrant transformer errors
        return BookLauncher.getClassNodeMixin(name, runTransformers, 0);
    }

    @Override
    public URL[] getClassPath() {
        return BookLauncher.getURLs();
    }

    @Override
    public Class<?> findClass(String name) throws ClassNotFoundException {
        return BookLauncher.loadClass(name, true);
    }

    @Override
    public Class<?> findClass(String name, boolean initialize) throws ClassNotFoundException {
        return BookLauncher.findClass(name, initialize);
    }

    @Override
    public Class<?> findAgentClass(String name, boolean initialize) throws ClassNotFoundException {
        return Class.forName(name, initialize, BookLoader.class.getClassLoader());
    }
    ...
    public void beginPhase() {
        BookMixinBootstrap.getProxyManager().createProxy();
    }
    ...
    public InputStream getResourceAsStream(String name) {
        return BookLauncher.getResourceAsStream(name);
    }
    ...
    public boolean isClassLoaded(String className) {
        return BookLauncher.isClassLoaded(className);
    }
    ...
    public String getSideName() {
        return BookLauncher.getEnvironment().name();
    }
